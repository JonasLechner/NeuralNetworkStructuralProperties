...
public class LongShortRotatingEdgeConstructor {
    ...
    public static void main(String[] args) {
        ...
        int outputNodes = 5;
        double sparsity = 0.4;
        StringBuilder stringBuilder = new StringBuilder("testing/LongShortRotatingEdgeConstructor" + inputNodes + "_");
        ...
        try {
            ...
            System.out.println("LongShortRotatingEdgeConstructor network generated successfully in " + outputFile);
        } catch (IOException e) {
            System.err.println("Error generating LongShortRotatingEdgeConstructor network: " + e.getMessage());
        }
    }
    ...
    private static List<Integer> generateIndices(int range, double sparsity) {
        ...
        int k = (int) Math.ceil(range*sparsity);

        int kFirstHalf = (int) Math.ceil((double) k /2);
        int kSecondHalf = k - kFirstHalf;

        for (int i = 0; i < kFirstHalf; i++) {
            indicesArr[i] = 1;
        }

        for (int i = kFirstHalf + 1; i < range; i+= (range - kFirstHalf) / kSecondHalf) {
            indicesArr[i] = 1;
        }
